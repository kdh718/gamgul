...
def q15(i): #원 둘레 넓이 문제
    ...
    if iidx%2 == 0:
        if type_of_square[idx]==0:
            str1="한 대각선의 길이가 %dcm인 정사각형을 둘러싼 원의 둘레를 구하시오."%(2*r)
            ans = 2*r*3.14
        else:
            str1="원을 둘러싼 정사각형의 한 변의 길이가 %dcm일 때 원의 둘레를 구하시오."%(2*r)
            ans = 2*r*3.14
        ans = "%.2fcm"%ans
    else:
        if type_of_square[idx]==0:
            str1="한 대각선의 길이가 %dcm인 정사각형을 둘러싼 원의 넓이를 구하시오."%(2*r)
            ans = r*r*3.14
        else:
            str1="원을 둘러싼 정사각형의 한 변의 길이가 %dcm일 때 원의 넓이를 구하시오."%(2*r)
            ans = r*r*3.14
        ans = "%.2fcm2"%ans
    while True:
...
def q21(i): #겹친 길이 구하기
    while True:
        a = random.randint(11,90) #큰 애
        b = random.randint(11,90) #작은 애
        if a<b: a,b=b,a
        elif a==b: continue
        if a%10!=0 and b%10!=0: break
    c = random.randint(5,25)
    a/=10; b/=10
    str1 = "색테이프 %.1fcm %d장을 %.1fcm씩 겹쳐서 붙였을 때 전체 길이는 몇 cm인가?"%(a,c,b)
    output = a*c - b*(c-1)
    ans = "%gcm"%output
    while True:
        input_ans = input("%d. %s\n"%(i, str1))
...
def choic_function(n):
    if n==0: return 1
    else: return 2

def q22(i): #공 튀어오르기
    a = random.randint(30,80) #높이
    b = random.randint(5,9) #튀어오르는 비율
    c = random.randint(2,4) # 바닥에 닿은 횟수
    b/=10
    str1 = "떨어진 높이의 %.1f만큼 튀어오르는 공이 있을 때, 이 공을 %dm에서 떨어뜨려 바닥에 %d번 닿을 동안 움직인 거리는 몇 m인가?"%(b,a,c)
    output = 0
    for i in range(c):
        output += (a * (b**i))*choic_function(i)
    ans = "%gm"%output
    while True:
        input_ans = input("%d. %s\n"%(i, str1))
        if input_ans == ans:
...
title = "*문제 생성기! V1.0.1 현재 등록된 문제 유형은 21가지입니다.*"
print(title)
print()
print('추후에 추가 유형과 편의 기능을 업데이트할 예정입니다. 아래 링크로 들어오셔서 최신버전을 확인하고 다운로드 받아주기 바랍니다.\n')
print('https://github.com/kdh718/gamgyul')
print()
print('문제 푸는 도중 정답인 것 같은데도 다음 문제로 안넘어가거나 프로그램이 종료되는 문제가 발생할 시 아래 메일로 해당 문제 유형을 알려주시기 바랍니다.\n')
print('kim.dohyun718@gmail.com')
print()
print('문제 시작합니다!\n')
print('*'*50)
print()
for i in range(1,101):
    q = [q1,q2,q3,q4,q5,q6,q7,q8,q9,q10,q12,q13,q14,q15,q16,q17,q18,q19,q20,q21,q22]
    random.choice(q)(i)
    print()
input('오늘의 문제를 모두 풀었습니다! 아무 키나 누르면 프로그램이 종료됩니다.')